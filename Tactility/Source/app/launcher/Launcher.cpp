...
static void onAppFocused(TT_UNUSED lv_event_t* e) {
    auto* parent = (lv_obj_t*)lv_event_get_user_data(e);
    
    lv_obj_t* apps_button = (lv_obj_t*)lv_event_get_target(e);
    lv_obj_t* img = lv_obj_get_child(apps_button, 0);

    lv_obj_t* wrapper = lv_obj_get_parent(apps_button);
    lv_obj_t* label = lv_obj_get_child(wrapper, 1);

    //lv_obj_set_style_image_recolor(img, lv_theme_get_color_secondary(parent), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(label, lv_theme_get_color_secondary(parent), 0);
}

static void onAppDefocused(TT_UNUSED lv_event_t* e) {
    auto* parent = (lv_obj_t*)lv_event_get_user_data(e);
    ...
    //lv_obj_set_style_image_recolor(img, lv_theme_get_color_primary(parent), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(label, lv_theme_get_color_primary(parent), 0);
}
...
static lv_obj_t* createAppButton(lv_obj_t* parent, const char* title, const char* imageFile, const char* appId, int32_t buttonPaddingLeft) {
    ...
    lv_obj_set_style_pad_bottom(apps_button, 20, 0);
    ...
    lv_obj_add_event_cb(apps_button, onAppFocused, LV_EVENT_FOCUSED, &parent);
    lv_obj_add_event_cb(apps_button, onAppDefocused, LV_EVENT_DEFOCUSED, &parent);
    ...
    lv_obj_set_size(button_image, 69, 69);
